...
Payloads can also define reusable `symbols` made from the same safe primitives. A `symbol` item references one of those definitions by id and places it in the schematic. Symbol definitions are payload data, not a hardcoded domain-specific symbol library. Definitions may include optional `parts` and `entitySlots` metadata so a future editor can describe symbol internals. Items inside a symbol definition may use `partId` to say which part they belong to. Symbol definitions may also include `partStyles`, which apply safe conditional styles to one part, and `partAnimations`, which apply safe animation presets such as `blink`, `pulse`, or `rotate` to one part. Symbol instances may include `slotBindings` to map generic slots such as `running` or `alarm` to concrete Home Assistant entity ids. Inside a symbol definition, existing condition fields can reference a slot with `slot:alarm` or `slot:running`.
